...
M3M4 "1" *-- "1..*" MetroLine : contains
 
MetroLine "1" *-- "2..*" LineStop : stops
 
M3M4 "1" *-- "0..*" Scenario : scenarios
...
**ATD - Alternativ TogDrift M3-M4 v2.1**  
...
ScenarioActivation:
activationId = "STK-OSO-2026-09-23-001"
```
...
### Source(assumption)
**ATD - Alternativ TogDrift M3-M4 v2.1, page 6**
...
**ATD - Alternativ TogDrift M3-M4 v2.1(boxes containing STW information)**
...
**assumption based on - Case A PowerPoint, slides 5–7**
...
The operating solution is present in **ATD - Alternativ TogDrift M3-M4 v2.1**.
...
| Attribute | Meaning | Traceability |
|---|---|---|
| `operationType : OperationType` | Type of service, e.g. pendulum. | Pendulum operation is explicitly present in the ATD - Alternativ TogDrift M3-M4 v2.1 power point. |
| `routeCode : String` | Route/section, e.g. `STK-OSO`. | Derived from the operating-plan material. |
...
| `did : String` | Destination ID used for the train operation. | Present in the ATD - Alternativ TogDrift M3-M4 v2.1 power point. |
...
`PENDULUM` is supported by the ATD - Alternativ TogDrift M3-M4 v2.1 power point/fallback material.

`ROUNDTRIP` should remain marked for confirmation unless a specific M3/M4 source explicitly uses that term.
...
**ATD - Alternativ TogDrift M3-M4 v2.1, page 1**
